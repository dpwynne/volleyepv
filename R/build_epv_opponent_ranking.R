...
build_epv_opponent_ranking <- function(my.files) {
  library(tidyverse)
  library(datavolley)
  library(readxl)
  ...
  fit_input_func <- function(a){
    fit_input <<- try(lm(rally_winner ~ poly(input_coord_x,2) + poly(input_coord_y,2) + input_kcode + rank_opponent, a), silent = TRUE)
    if (class(fit_input) == "try-error") {
      fit_input <<- lm(rally_winner ~ poly(input_coord_x,2) + poly(input_coord_y,2) + rank_opponent, a)
    }
  }

  b <- subset(epv_data, skq == "Set #" & !is.na(output_coord_x))
  fit_output_func <- function(b){
    fit_output <<- try(lm(rally_winner ~ poly(output_coord_x,2) + poly(output_coord_y,2) + output_kcode + rank_opponent, b), silent = TRUE)
    if (class(fit_output) == "try-error") {
      fit_output <<- lm(rally_winner ~ poly(output_coord_x,2) + poly(output_coord_y,2) + rank_opponent, b)
    }
  }

  fit_setoutput_func <- function(a){
    fit_set_output <<- try(lm(rally_winner ~ poly(input_coord_x,2) + poly(input_coord_y,2) + input_kcode + blockers + rank_opponent, a), silent = TRUE)
    if (class(fit_set_output) == "try-error") {
      fit_set_output <<- lm(rally_winner ~ poly(input_coord_x,2) + poly(input_coord_y,2) + blockers + rank_opponent, a)
    }
  }
  ...
  serve_baseline_model <- function(epv_data) {
    tryCatch (
      {serve_baseline_model <- glm(rally_winner ~ rank_opponent, subset(epv_data, input_type == "serve_baseline"), family = "binomial")},
  ...
  attack_overpass_model <- function(epv_data) {
    tryCatch (
      {attack_overpass <- glm(rally_winner ~ rank_opponent, subset(epv_data, input_type == "attack_overpass"), family = "binomial")},
      error = function(error) {
        return(NA)
      }
    )
  ...
  attack_weird_model <- function(epv_data) {
    tryCatch (
      {attack_weird <- glm(rally_winner ~ rank_opponent, subset(epv_data, input_type == "attack_weird"), family = "binomial")},
      error = function(error) {
        return(NA)
      }
    )
  ...
  attack_poor_set_model <- function(epv_data) {
    tryCatch (
      {attack_poor_set <- glm(rally_winner ~ rank_opponent, subset(epv_data, input_type == "attack_poor_set"), family = "binomial")},
      error = function(error) {
        return(NA)
      }
    )
  ...
  block_weird_model <- function(epv_data) {
    tryCatch (
      {block_weird <- glm(rally_winner ~ rank_opponent, subset(epv_data, input_type == "block_weird"), family = "binomial")},
      error = function(error) {
        return(NA)
      }
    )
  ...
  dig_overpass_model <- function(epv_data) {
    tryCatch (
      {dig_overpass <- glm(rally_winner ~ rank_opponent, subset(epv_data, input_type == "dig_overpass"), family = "binomial")},
      error = function(error) {
        return(NA)
      }
    )
  ...
  dig_weird_model <- function(epv_data) {
    tryCatch (
      {dig_weird <- glm(rally_winner ~ rank_opponent, subset(epv_data, input_type == "dig_weird"), family = "binomial")},
      error = function(error) {
        return(NA)
      }
    )
  ...
  freeball_baseline_model <- function(epv_data) {
    tryCatch (
      {freeball_baseline <- glm(rally_winner ~ rank_opponent, subset(epv_data, input_type == "freeball_baseline"), family = "binomial")},
      error = function(error) {
        return(NA)
      }
    )
  ...
  set_poor_model <- function(epv_data) {
    tryCatch (
      {set_poor <- glm(rally_winner ~ rank_opponent, subset(epv_data, output_type == "set_poor"), family = "binomial")},
      error = function(error) {
        return(NA)
      }
    )
  ...
  block_touch_stuff_model <- function(epv_data) {
    tryCatch (
      {block_touch_stuff <- glm(rally_winner ~ rank_opponent + speed + block_time, subset(epv_data, skill=="Dig"), family = "binomial")},
      error = function(error) {
        return(NA)
      }
    )
